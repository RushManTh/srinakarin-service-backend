...
export async function upsertMontessoriLearned(data: {
  ...
  // Optional validation: ensure student and activity exist
  const [student, activity] = await Promise.all([
    prisma.student.findUnique({ where: { id: data.studentId } }),
    prisma.montessoriActivity.findUnique({ where: { id: data.activityId } }),
  ]);
...
export async function toggleMontessoriLearned({
  ...
  const [student, activity] = await Promise.all([
    ...
    prisma.montessoriActivity.findUnique({ where: { id: activityId } }),
  ]);
  if (!student) throw new Error("Student not found");
  if (!activity) throw new Error("Activity not found");
...
export async function listLearnedByStudentAndSchoolSubject({
  studentId,
  schoolSubjectId,
}: {
  studentId: string;
